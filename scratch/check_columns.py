...
for name in ["cr5db_approvalroutes", "cr5db_changerequests"]:
    print(f"\n=== Columns in {name} ===")
    url = f"{API_URL}/EntityDefinitions(LogicalName='{name}')/Attributes?$select=LogicalName,AttributeType"
    r = requests.get(url, headers=headers)
    if r.status_code == 200:
        attrs = r.json().get("value", [])
        custom_attrs = [a for a in attrs if a["LogicalName"].startswith("cr5db_")]
        for a in sorted(custom_attrs, key=lambda x: x["LogicalName"]):
            print(f"  - {a['LogicalName']} ({a['AttributeType']})")
    else:
        print(f"Failed to fetch attributes: {r.status_code} - {r.text[:200]}")
