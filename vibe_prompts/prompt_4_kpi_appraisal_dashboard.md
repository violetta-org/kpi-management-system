...
1. Create a "My KPIs" dashboard tab for standard employees (visible if `currentUserRecord.cr5db_SystemRole` is "Employee"):
...
2. Create a "My Appraisals" tab:
   - Display a list of the employee's "Performance Appraisal" (cr5db_PerformanceAppraisal) records (filtered by cr5db_EmployeeID matching currentUserRecord.cr5db_UserId).
...
3. Create a "Team Appraisals" manager review panel:
   ...
   - Display pending appraisals for their team members (where the logged-in user is set as the cr5db_EvaluatorID, or they are the manager in the job position hierarchy).
   - Let managers enter the final score (cr5db_FinalScore) and manager comments.
4. Create an "Appraisal Administration" tab (visible only if system role is "HRManager" or "Admin"):
   - Allows HR to create new Evaluation Periods, set KPI Targets for employees, and view final consolidated Performance Appraisals for the entire company.
5. Auto-Calculate Score Aggregations:
